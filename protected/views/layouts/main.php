...
<html lang="en">
  <head>
	...
	<link rel=icon href="<?php echo Yii::app()->theme->baseUrl; ?>/img/cs1.6.png">
	...
	<div id="nav" class="clearfix">
		<div class="container">
			<ul class="main-nav">
				...
				<?php endforeach; //echo CHtml::dropDownList('Other', 'other', Usermenu::getMenu(true));?>
				<?php if(!empty(Usermenu::getMenu(true))): ?>
				<li class="dropdown">
				...
				<ul class="dropdown-menu">
					<?php
						foreach( Usermenu::getMenu(true) as $item )
						{
							echo "<li>";
							echo CHtml::link(
								CHtml::encode($item['label']),
			...
			<ul class="main-nav pull-right">
				<li class="dropdown">
						...
						<li>
							<p>
								<form method="post" action="<?php echo Yii::app()->createUrl('/site/login')?>" accept-charset="UTF-8">
									<input style="margin-bottom: 15px;" type="text" placeholder="Login" autocomplete="username" name="LoginForm[username]">
									<input style="margin-bottom: 15px;" type="password" placeholder="Password" autocomplete="current-password" name="LoginForm[password]">
									<input type="hidden" value="<?php echo Yii::app()->request->csrfToken?>" name="<?php echo Yii::app()->request->csrfTokenName?>" />
									<input class="btn btn-primary btn-block" name="yt0" type="submit" value="Login">
								</form>
							</p>
						</li>
					</ul>
					...
					<ul class="dropdown-menu">
						<li>
							<?php echo CHtml::link(
									'<i class="icon-globe"></i> Admin Panel',
