...
class InstallForm extends CFormModel
{
	...
	public function testConnect($close = TRUE, $minVer = '5.0') {
		...
		if($minVer) {
			if(version_compare($this->conn->serverVersion, $minVer, '<')) {
				return 'DB server version below recommended, minimum ' . $minVer;
			}
		}
	...
	public function installDB() {
		...
		$cfg = '<?php' . PHP_EOL;
		$cfg .= '/**' . PHP_EOL;
		$cfg .= ' * System Configuration Data' . PHP_EOL;
		$cfg .= ' *' . PHP_EOL;
		$cfg .= ' * @author Craft-Soft Team' . PHP_EOL;
		$cfg .= ' * @version 1.0 beta' . PHP_EOL;
		$cfg .= ' * @copyright (C)2013 Craft-Soft.ru.  All rights reserved.' . PHP_EOL;
	...
	protected function afterValidate() {
		...
		if(($err = $this->testConnect()) !== TRUE) {
			$this->addError('', 'Error connecting to the database: ' . $err);
		}

		if(!$this->license) {
			$this->addError('license', 'You have not accepted the terms of the license agreement');
		}
