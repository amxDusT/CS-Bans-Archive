...
class Webadmins extends CActiveRecord
{
			...
			(
				...
				(
					$model->levels->$access === 'own'
						&&
					(strtolower($username) === strtolower(Yii::app()->user->name) || strtolower($username) === strtolower("amx_".Yii::app()->user->name))
				)
			)
		) {
			return TRUE;
	...
	public function afterSave() {
		if($this->isNewRecord)
			Syslog::add(Logs::LOG_ADDED, 'Added new web admin <strong>' . $this->username . '</strong>');
		elseif($this->scenario !== 'auth')
			Syslog::add(Logs::LOG_EDITED, 'Web admin details changed <strong>' . $this->username . '</strong>');
		return parent::afterSave();
	}

	public function afterDelete() {
		Syslog::add(Logs::LOG_DELETED, 'Web admin removed <strong>' . $this->username . '</strong>');
