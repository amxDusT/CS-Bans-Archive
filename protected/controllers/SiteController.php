...
class SiteController extends Controller
{
	...
	public function actionInstall() {
		...
		if(Yii::app()->db->username) {
			throw new CHttpException(404, 'The system is already installed.');
		}
		...
		if(Yii::app()->request->isAjaxRequest) {
			...
			Yii::app()->end('<span class="text-success">Connection established</span>');
		}
		...
		if (version_compare(PHP_VERSION, $minPhp, '<')) {
			$error[] = 'You are using a PHP version below the recommended';
		}

		foreach($testFunc AS $func) {
			if(!function_exists($func) || in_array($func, $disFunc)) {
				$error[] = "Feature not available {$func}";
			}
		}

		foreach($testExt AS $ext) {
			if(!extension_loaded($ext)) {
				$error[] = "Extension not available {$ext}";
			}
		}

		if(!is_writable($confFile)) {
			if(!chmod($confFile, 0666)) {
				$error[] = 'Insufficient permissions to write to config';
			}
		}
		if(!is_writable($assetDir)) {
			if(!chmod($assetDir, 0777)) {
				exit('Insufficient permissions to write to the /assets folder');
			}
		}
		if(!is_writable($runtimeDir)) {
			if(!chmod($runtimeDir, 0777)) {
				exit('Insufficient permissions to write to /protected/runtime folder');
			}
		}
	...
	public function actionUpdate() {
		...
		if(Yii::app()->user->isGuest) {
			throw new CHttpException(403, 'Figwam');
		}
		...
		if($info[0]['start_page'] === '/site/index') {
			...
			throw new CHttpException(404, 'No Updates');
		}
